...
// Get -tn images based on the invId from images
function getImagesTn($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT classificationId, invColor, invDescription, inventory.invId, invModel, invMake, invPrice, invStock, images.imgPath FROM inventory JOIN images ON inventory.invId = images.invId WHERE inventory.invId = :invId AND images.imgPath LIKE "%-tn%"';
    $stmt = $db->prepare($sql); 
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT); 
    $stmt->execute(); 
